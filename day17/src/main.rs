...
fn part1(start: &Dimension) {
    ...
    println!("Part 1: {}", dimension.active_cells.len());
}

fn part2(start: &Dimension) {
    let start = start.clone();
    ...
    println!("Part 2: {}", dimension.active_cells.len());
}

fn main() {
    let active_cells = include_str!("../input.txt")
        .lines()
        .enumerate()
        .flat_map(|(y, line)| {
            line.chars()
    ...
    let start = Dimension { active_cells };
    part1(&start);
    part2(&start);
}
